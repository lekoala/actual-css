...
```css
:where(.actions, .nav-list) {
  margin: 0;
  padding: 0;
  list-style: none;
}

:where(.actions, .nav-list) > li {
  margin: 0;
}
...
Import it after tokens and layout, before components.
...
This lets components rely on shared structural patterns without owning their reset or layout behavior.
