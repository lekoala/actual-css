...
- `--choice-control-nudge` — block correction added to the derived first-line
  offset, for every control in the row. Left unset, each control centres itself
  on the first line's cap height, measured from its own block size — so a
  checkbox and a taller switch stay level with each other, and with the text,
  whatever the font, line-height or control size.
