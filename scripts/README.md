...
| `report:overflow` | `report-overflow.js` | Survey demo and site pages at phone width, never failing — which ones let the viewport scroll sideways, bisected down to the element that owns it. Asks whether the viewport really scrolls, not whether `scrollWidth` is large, so a correctly clipped scroll container is never reported. Demo pages link `dist/`, so build first. |
